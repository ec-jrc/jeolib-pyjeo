...
class BadBasicMethods(unittest.TestCase):
    ...
    @staticmethod
    def test_jim_open():
        """Test opening Jim objects."""
        jim1 = pj.Jim()
        jim1.io.open(tiles[0])
        jim2 = pj.Jim()
        jim2.io.open(jim1, copy_data=True)
        jim3 = pj.Jim()
        jim3.io.open(jim1, copy_data=False)

        assert jim1.properties.isEqual(jim2), 'Error in creating Jim object'
        assert not jim1.properties.isEqual(jim3), \
            'Error in opening Jim object'

        jim4 = pj.Jim()
        jim4.io.open(jim1, nrow=5)

        assert jim1.properties.isEqual(jim4), \
            'Error in ignoring kwargs when opening Jim object with ' \
            'Jim(jim, kwargs)'

        jim5 = pj.Jim()
        jim5.io.open(tiles[0], nrow=5)

        assert jim5.properties.nrOfRow() == 5, \
            'Error in opengin Jim with Jim(filepath, kwargs)'
        assert jim5.properties.nrOfCol() == jim1.properties.nrOfCol(), \
            'Error in opening Jim with Jim(filepath, kwargs)'

        jim6 = pj.Jim()
        jim6.io.open(nrow=5, ncol=5)

        assert jim6.properties.nrOfRow() == 5, \
            'Error in opening Jim with Jim(kwargs)'
        assert jim6.all.nrOfCol() == 5, \
            'Error in opening Jim with Jim(kwargs)'

        jim7 = pj.Jim()
        jim7.io.open(nrow=50, ncol=50, otype='Float32', uniform=[0, 2],
                      seed=0)
        stats7 = jim7.stats.getStats(['min', 'max'], band=0)

        assert stats7['max'] < 2 and stats7['min'] > 0, \
            'Error in opening Jim with uniform distribution'

        jim8 = pj.Jim()
        jim8.io.open(nrow=50, ncol=50, otype='Float32', uniform=[0, 2],
                      seed=0)

        assert jim7.properties.isEqual(jim8), \
            'Error in usage of seed keyword argument when opening Jim()'

        jim8_2 = pj.Jim()
        jim8_2.io.open(nrow=50, ncol=50, otype='Float32', uniform=2,
                        seed=0)

        assert jim7.properties.isEqual(jim8_2), \
            'Error in using only one value for uniform open of Jim()' \
            '(created Jim is not the same as the one created with[0, value])'
        ...
        assert raised, \
            'Error in catching a call of Jim open with wrong value ' \
            'parsed as the uniform argument (three values parsed)'

        # Test open with mean and stdev

        jim9 = pj.Jim()
        jim9.io.open(nrow=50, ncol=50, otype='Float32', mean=500, stdev=50,
                      seed=0)

        stats = jim9.stats.getStats(['mean', 'stdev'])

        assert 500 - 50 < stats['mean'] < 500 + 50, \
            'Suspicious values in Jim when opening it with Jim(mean, stdev)' \
            '(mean seems too distant from the specified one)'
        assert 50 - 5 < stats['stdev'] < 50 + 5, \
            'Suspicious values in Jim when opening it with Jim(mean, stdev)' \
            '(stdev seems too different from the specified one)'

        # Test open with only seed (mean=0, stdev=1)

        jim10 = pj.Jim()
        jim10.io.open(nrow=50, ncol=50, otype='Byte', seed=0)

        jim10_2 = pj.Jim()
        jim10_2.io.open(nrow=50, ncol=50, otype='Byte', mean=0, stdev=1,
                         seed=0)

        assert jim10.properties.isEqual(jim10_2), \
            'Error when opening Jim with Jim(nrow, nncol, otype, seed) ' \
            '(not equal to Jim(nrow, nncol, otype, seed, mean=0, stdev=1))'
        ...
        assert raised, \
            'Error in catching a call of Jim open with nonsense (kw)args'
        ...
        assert raised, \
            'Error in catching a call of Jim open with non-existing path'
